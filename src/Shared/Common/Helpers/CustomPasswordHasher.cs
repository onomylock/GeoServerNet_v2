    ...
    public bool VerifyPassword(string hashedPassword, string enteredPassword)
    {
        ...
        try
        {
            if (_includeHeaderInfo)
            {
                ...
                var iterCountRead = (int)ReadNetworkByteOrder(decodedHashedPassword, 5);
                if (_iterCount != iterCountRead) return false;

                var saltLengthRead = (int)ReadNetworkByteOrder(decodedHashedPassword, 9);
            ...
            var subkeyLength = decodedHashedPassword.Length - headerByteLength - salt.Length;

            if (_requestedLength != subkeyLength) return false;

            var expectedSubkey = new byte[subkeyLength];
            Buffer.BlockCopy(decodedHashedPassword, headerByteLength + salt.Length, expectedSubkey, 0,
