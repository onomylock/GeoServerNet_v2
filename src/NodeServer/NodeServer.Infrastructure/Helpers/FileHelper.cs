...
public static class FileHelper
{
    ...
    public static readonly string HomePath =
        Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), HomeDirectory);
    ...
    {
        return Directory.CreateDirectory(Path.Join(ResultsPath, Path.GetDirectoryName(solutionPath)));
    }

    public static DirectoryInfo CreateResultPath(string resultsPath, Guid solutionId)
    {
        return Directory.CreateDirectory(Path.Combine(resultsPath,
    ...
    {
        return Directory.CreateDirectory(Path.Join(WorkingDirectory,
            workingPath + DateTimeOffset.UtcNow.ToUnixTimeSeconds()));
    }
}